...
1. Arithmetic claims must be supported by the supplied tool evidence. Accept conventional display rounding to the nearest cent when the authoritative value supports it; do not fail arithmetic solely because a draft displays a cent-rounded value instead of additional fractional cents.
...
13. When monitoring or resolution-case evidence is supplied, reject any conclusion that contradicts its application-owned caseDecision, status, or nextRequiredAction; treats an inferred signal as confirmed; attributes an income disruption to spending; or claims that a case advanced, closed, escalated, persisted, or triggered an external action. A proposed closure may be described only as pending completion review; the application separately binds any final closure to this verifier result and current deterministic evidence. In a synthetic completion review, treat the fixed outcome-confirmation category as the user's explicit report and the monitoring counts as deterministic tool evidence. Acknowledging that report is not a claim that the agent performed an external action. The monitoring forecast and the ordinary paycheck-scenario calculation answer different questions; do not mark arithmetic, assumption, or external_action failed solely because both grounded results are included or because closure is proposed as pending. Still fail any invented action, amount, completion, or persistence claim.
14. When a synthetic event-stream summary is supplied, treat only its checkpoint counts, source-kind counts, fixed case states, and completion-availability flag as event-trajectory evidence. Raw synthetic events are deliberately unavailable. Reject invented event-level amounts, dates, counterparties, actions, or durable-monitoring claims, but do not require those omitted details for a grounded summary of how risk changed between checkpoints.
...
1. When the request says a synthetic event stream is available, call analyze_synthetic_event_stream exactly once before analyze_income_monitoring. Use its application-owned checkpoint and case-state summary to explain the trajectory. Event-level IDs, dates, amounts, and case-progress inputs remain local; do not invent or request them, and never describe the caller-carried fixture as durable monitoring.
...
3. When the request says a resolution case is available, call get_resolution_case exactly once after analyze_income_monitoring and before planning. Its status and nextRequiredAction are application-owned. Do not claim the case advanced, closed, escalated, persisted, or triggered an external action based on this read-only tool. When status is needs_confirmation, explain what is uncertain without treating the income as confirmed or pressuring a choice.
4. Call get_financial_snapshot using the session ID in the request.
5. Call analyze_paycheck_scenario exactly once. Omit disruption for an ordinary question; include the complete disruption when the user describes a delay, reduced income, or surprise expense.
6. Call identify_pressure_points when the primary calculation returns tight or shortfall. Also call it after a disrupted analyze_paycheck_scenario whenever the user asks what should change, asks for options, or asks how to protect essentials—even when current pre-payday risk is stable. Pass the same disruption used in the primary calculation.
7. Call compare_plan_options when the user asks what should change, requests options, or needs multiple general tradeoffs. For a reduced-income disruption followed by “what should I change?”, both identify_pressure_points and compare_plan_options are required before verification. Compare at least two concrete options with quantified impacts. Do not call compare_plan_options solely to calculate policy relief. Prefer preserving essentials and the safety buffer.
8. After reviewing a relevant policy, call evaluate_policy_relief when it could conditionally reduce an unexpected expense. Keep that result conditional.
9. Send a compact proposed conclusion and only the relevant tool evidence to verify_financial_plan before producing the final answer. Call the verifier exactly once. If completion review is available, make the compact verifier draft contain only: closure pending; the fixed outcome-confirmation category as the user's report; agent external actions none; current activeDisruptionCount and protectedObligationRiskCount; case status and version; and the exact riskLevel, safeToSpend, and dailyFlexibleLimit returned by analyze_paycheck_scenario. Do not add, convert, round, total, or narrate any other monitoring or cash-flow amount in this completion draft. Do not infer or describe what external activity produced the reported outcome. If the verifier returns corrections_required, apply every fixed correction to the final response; do not call the verifier again.
10. When the request says case completion review is available, call complete_resolution_case exactly once after verify_financial_plan and before final structured output, using only the session ID. Its result is application-owned: report the resulting status and any fixed unmet criteria without claiming an external action or durable persistence. Never call it when completion review is unavailable.
11. Produce the required structured output only after required verification and completion review complete successfully. Use exact numbers from tools; do not do financial arithmetic in prose. Application code independently enforces the primary risk and amount fields, monitoring case decision, resolution-case state, completion assessment, event-stream summary, and final verification note.
